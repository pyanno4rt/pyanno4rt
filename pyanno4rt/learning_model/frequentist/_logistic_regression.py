...
class LogisticRegressionModel():
    ...
    logistic regression model from the input data, inspect the model, make \
    ...
    ----------
    model_label : str
        Label for the logistic regression model to be used for file naming.
    ...
    prediction_model : object of class `LogisticRegression`
        Instance of the class `LogisticRegression`, which holds methods to \
    ...
    def __init__(
        ...
        self.configuration = {
            ...
            'tune_space': {
                ...
                'penalty': tune_space.get('penalty', ['l1', 'l2']),
    ...
    def get_model(
            ...
            labels):
        """
        Get the logistic regression outcome prediction model by reading from \
        the model file path, the datahub, or by training.

        Returns
        -------
        object of class `LogisticRegression`
            Instance of the class `LogisticRegression`, which holds methods \
            to make predictions from the logistic regression model.
        """
    ...
    def tune_hyperparameters(
            ...
            labels):
        """
        Tune the hyperparameters of the logistic regression model via \
        ...
        def objective(proposal, trials, space):
            ...
            hyperparameters = {**proposal['regularization'],
                               'tol': proposal['tol'],
                               'fit_intercept': True,
        ...
        space = {
            'regularization': hp.choice('regularization', [
                *[{'penalty': norm, 'C': hp.uniform(
                    f'C_{norm}', self.configuration['tune_space']['C'][0],
                    self.configuration['tune_space']['C'][1])}
                  for norm in self.configuration['tune_space']['penalty']]]),
            'tol': hp.choice('tol', self.configuration['tune_space']['tol']),
    ...
    def train(
        ...
        # Create a dictionary with the model hyperparameters
        hyperparameters = {**tuned_hyperparameters['regularization'],
    ...
    def predict_oof(
        ...
        def compute_fold_labels(indices):
            ...
            # Initialize the model from the hyperparameter dictionary
            prediction_model = LogisticRegression(**self.hyperparameters)
    ...
    def read_model_from_file(self):
        """
        Read the logistic regression outcome prediction model from the model \
        file path.
    ...
    def write_model_to_file(
            ...
            prediction_model):
        """
        Write the logistic regression outcome prediction model to the model \
        file path.
    ...
    def read_hyperparameters_from_file(self):
        ...
        dict
            ...
            logistic regression outcome prediction model.
        """
