"""Extreme gradient boosting outcome prediction model."""
...
class ExtremeGradientBoostingModel():
    """
    Extreme gradient boosting outcome prediction model class.

    This class enables building an individual preprocessing pipeline, fit the \
    extreme gradient boosting model from the input data, inspect the model, \
    make predictions with the model, and assess the predictive performance \
    ...
    ----------
    model_label : string
        Label for the extreme gradient boosting model to be used for file \
        naming.
    ...
    dataset : dict
        ...
        for the extreme gradient boosting model model.

    preprocessing_steps : tuple
        Sequence of labels associated with preprocessing algorithms which \
        make up the preprocessing pipeline for the extreme gradient boosting \
    ...
    tune_space : dict
        ...
        - 'booster' : specification of the booster to use;
    ...
    oof_splits : int
        ...
        out-of-folds evaluation step of the extreme gradient boosting model.
    ...
    model_path : string
        Path for storing and retrieving the extreme gradient boosting model.
    ...
    prediction_model : object of class `XGBClassifier`
        Instance of the class `XGBClassifier`, which holds methods \
        to make predictions from the extreme gradient boosting model.
    ...
    def __init__(
        ...
        self.configuration = {'data': [dataset['feature_values'].shape[0],
                              ...
                              'tune_space': {
                                  ...
                                  'grow_policy': tune_space.get(
                                      'grow_policy', ['depthwise',
                                                      'lossguide']),
                                  'learning_rate': tune_space.get(
                                      'learning_rate', [1e-5, 1e-2]),
                                  'booster': tune_space.get(
        ...
        # Get the extreme gradient boosting model and its hyperparameters
        ...
        if (inspect_model and (self.updated_model or self.model_label
            ...
            self.inspector = ModelInspector(
                ...
                model_class='Extreme Gradient Boosting')
        ...
        if (evaluate_model and (self.updated_model
                                or self.model_label
    ...
    def get_prediction_model(self):
        """
        Get the extreme gradient boosting outcome prediction model by reading \
        from the model file path, the datahub, or by training.

        Returns
        -------
        object of class `XGBClassifier`
            Instance of the class `XGBClassifier`, which holds methods to \
            make predictions from the extreme gradient boosting model.
        """
    ...
    def read_model_from_file(self):
        """
        Read the extreme gradient boosting outcome prediction model from the \
        model file path.
    ...
    def write_model_to_file(
            ...
            prediction_model):
        """
        Write the extreme gradient boosting outcome prediction model to the \
        model file path.
    ...
    def read_hyperparameters_from_file(self):
        """
        Read the extreme gradient boosting outcome prediction model \
    ...
    def tune_hyperparameters_with_bayes(self):
        """
        Tune the hyperparameters of the extreme gradient boosting model via \
        sequential model-based optimization using the tree-structured Parzen \
        estimator. As a variation, the objective function is evaluated based \
        on a stratified k-fold cross-validation.
        ...
        def objective(proposal, trials, space):
            ...
            prediction_model = XGBClassifier(**hyperparameters)

            # Compute the objective function value (loss) across all folds
            losses = ThreadPoolExecutor().map(
                compute_fold_score,
    ...
    def train(self):
        """
        Train the extreme gradient boosting outcome prediction model.
    ...
    def predict_oof(
        ...
        def compute_fold_labels(indices):
            ...
            # Initialize the model from the hyperparameter dictionary
            prediction_model = XGBClassifier(**self.hyperparameters)
