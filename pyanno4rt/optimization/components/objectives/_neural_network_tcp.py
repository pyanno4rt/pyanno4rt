...
from tensorflow.compat.v2 import cast, float64, GradientTape
...
class NeuralNetworkTCP(ModelObjectiveClass):
    ...
    neural network TCP objective, as well as to add the neural network model \
    and to get/set the parameters and the objective weight.
    ...
    model_parameters : dict
        ...
        - ``feature_filter`` : tuple, default = ((), 'remove'), (sub)set \
            ...
            prior to fitting the neural network model;
        ...
        - ``preprocessing_steps`` : tuple, default = ('Equalizer',), sequence \
            ...
            the preprocessing pipeline for the neural network model;
        ...
        - ``inspect_model`` : bool, default = False, indicator for the \
            inspection of the neural network model;
        ...
        - ``oof_splits`` : int, default = 5, number of splits for the \
            ...
            step of the neural network model;
    ...
    model : object of class `NeuralNetworkModel`
        Instance of the class `NeuralNetworkModel`, which holds methods to \
        preprocess, tune, train, inspect and evaluate the neural network model.

    parameter_value : list
        Value of the neural network parameters.
    """
    ...
    def __init__(
        ...
        super().__init__(name='Neural Network TCP',
    ...
    def add_model(self):
        """Add the neural network model to the objective."""
        # Initialize the datahub
        hub = Datahub()

        # Log a message about the model addition
        hub.logger.display_info("Adding {} neural network model for '{}' ..."
                                .format(self.model_parameters['architecture'],
                                        self.name))
    ...
    def compute_objective_value(
        ...
        neural network model (convex if the input-convex architecture is used).
        """
        ...
        return -self.model.predict(
            preprocessed_features, squash_output=False).numpy()
    ...
    def compute_gradient_value(
        ...
        # Introduce the gradient tape as 'tape'
        with GradientTape() as tape:

            # Watch the gradient operations on the input features
            tape.watch(preprocessed_features)

            # Get the model output from the features
            output = self.model.optimization_model(preprocessed_features)

            # Get the model gradient from the gradient tape
