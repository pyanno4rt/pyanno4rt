"""Extreme gradient boosting NTCP objective."""
...
from pyanno4rt.learning_model.frequentist import ExtremeGradientBoostingModel
...
class ExtremeGradientBoostingNTCP(MachineLearningObjectiveClass):
    """
    Extreme gradient boosting NTCP objective class.

    This class provides methods to compute the value and the gradient of the \
    extreme gradient boosting NTCP objective, as well as to add the extreme \
    gradient boosting model and to get/set the parameters and the objective \
    weight.
    ...
    model_parameters : dict
        ...
        - ``feature_filter`` : tuple, default = ((), 'remove'), (sub)set \
            ...
            prior to fitting the extreme gradient boosting model;
        ...
        - ``preprocessing_steps`` : tuple, default = ('Equalizer',), sequence \
            ...
            the preprocessing pipeline for the extreme gradient boosting model;
        ...
        - ``inspect_model`` : bool, default = False, indicator for the \
            inspection of the extreme gradient boosting model;
        ...
        - ``oof_splits`` : int, default = 5, number of splits for the \
            ...
            step of the extreme gradient boosting model;
    ...
    model : object of class `XGBClassifier`
        Instance of the class `XGBClassifier`, which holds methods to \
        preprocess, tune, train, inspect and evaluate the extreme gradient \
        boosting model.

    parameter_value : list
        Value of the extreme gradient boosting model parameters.
    """
    ...
    def __init__(
        ...
        super().__init__(name='Extreme Gradient Boosting NTCP',
                         parameter_name=(),
                         parameter_category=(),
    ...
    def add_model(self):
        """Add the extreme gradient boosting model to the objective."""
        # Initialize the datahub
        hub = Datahub()

        # Log a message about the model addition
        hub.logger.display_info("Adding extreme gradient boosting model for "
                                "'{}' ..."
                                .format(self.name))
        ...
        # Get the extreme gradient boosting model
        self.model = ExtremeGradientBoostingModel(
