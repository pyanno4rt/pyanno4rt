"""K-nearest neighbors NTCP objective."""
...
class KNeighborsNTCP(ModelObjectiveClass):
    ...
    k-nearest neighbors NTCP objective, as well as to add the k-nearest \
    neighbors model and to get/set the parameters and the objective weight.
    ...
    model_parameters : dict
        ...
        - ``feature_filter`` : tuple, default = ((), 'remove'), (sub)set \
            ...
            prior to fitting the k-nearest neighbors model;
        ...
        - ``preprocessing_steps`` : tuple, default = ('Equalizer',), sequence \
            ...
            the preprocessing pipeline for the k-nearest neighbors model;
        ...
        - ``inspect_model`` : bool, default = False, indicator for the \
            inspection of the k-nearest neighbors model;
        ...
        - ``oof_splits`` : int, default = 5, number of splits for the \
            ...
            step of the k-nearest neighbors model;
    ...
    model : object of class `KNeighborsModel`
        Instance of the class `KNeighborsModel`, which holds methods to \
        preprocess, tune, train, inspect and evaluate the k-nearest \
        neighbors model.
    ...
    def add_model(self):
        """Add the k-nearest neighbors model to the objective."""
        # Initialize the datahub
        hub = Datahub()

        # Log a message about the model addition
        hub.logger.display_info("Adding k-nearest neighbors model for '{}' ..."
                                .format(self.name))
        ...
        # Get the k-nearest neighbors model
        self.model = KNeighborsModel(
    ...
    def compute_gradient_value(
        ...
        # Preprocess the dose and segment inputs
        preprocessed_features = self.model.preprocess(raw_features)

        # Get the model gradient from the minimal improving point shift
        model_gradient = self.compute_minimal_shift(
            preprocessed_features, min_reduce=1e-3)
    ...
    def compute_minimal_shift(
            self,
            feature_vector,
            min_reduce=1e-3):
        """
        Compute the minimal improving point shift as an estimate to the \
        model gradient.

        Parameters
        ----------
        feature_vector : ndarray
            Reference vector in the feature space.

        min_reduce : int or float
            Minimum reduction in the model outcome probability prediction to \
            view the current solution as feasible.

        Returns
        -------
        ndarray
            Minimum improving point shift representing the model gradient.
        """

        @jit
        def objective(weights):
            """Compute the euclidean norm of the point shift."""
            return jnp.linalg.norm(weights)

        @jit
        def jacobian(weights):
            """Compute the gradient of the objective function."""
            return grad(objective)(weights)

        def probability_constraint(weights):
            """Compute the KNN prediction for the probability constraint."""
            return (self.model.prediction_model.predict_proba(
                feature_vector - weights[:, None].T)[:, 1][0]
                + min_reduce)

        # Initialize the weights vector
        initial_weights = zeros((self.model.features.shape[1],))

        # Optimize the improving point shift
        result = minimize(x0=initial_weights, fun=objective, method='SLSQP',
                          jac=jacobian, constraints=(NonlinearConstraint(
                              probability_constraint, lb=0,
                              ub=self.model.prediction_model.predict_proba(
                                  feature_vector)[:, 1][0])),
