    ...
    def translate(
        ...
        if self.transform:
            ...
            if isinstance(value, (tuple, list)):
                ...
                return [
                    sigmoid(sign*4*val-2, self.multiplier[0], self.summand[0])
                    if val > sign*0.5 else sign*val for val in value]

            # Return a single transformed outcome value
            return (
                sigmoid(sign*4*value-2, self.multiplier[0], self.summand[0])
                if value > sign*0.5 else sign*value)
    ...
    def reverse(
        ...
        if self.transform:
            ...
            if isinstance(value, (tuple, list)):
                ...
                return [
                    sign*0.25*inverse_sigmoid(
                        val, self.multiplier[0], self.summand[0])+sign*0.5
                    if sign*val > sign*0.5 else sign*val for val in value]

            # Return a single transformed function value
            return (
                sign*0.25*inverse_sigmoid(
                    value, self.multiplier[0], self.summand[0])+sign*0.5
