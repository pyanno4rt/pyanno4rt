...
class LowRankIntegrators:
    ...
    def update(
        ...
        #
        if self.integrator_name == "fixedrankBUG":

            #
            return self.fixedrankBUG_onestep(U, S, V, dt)

        #
        elif self.integrator_name == "fixedranksymmetricBUG":

            #
            return self.fixedranksymmetricBUG_onestep(U, S, V, dt)

        #
        elif self.integrator_name == "fixedaugBUG":

            #
            return self.fixedaugBUG_onestep(U, S, V, dt)

        #
        elif self.integrator_name == "augBUG":

            #
            return self.augBUG_onestep(U, S, V, dt)

        #
        elif self.integrator_name == "parBUG":
    ...
    def truncate(
        ...
        if fixed:
            ...
            return  U @ P[:,:self.rank], diag(D[:self.rank]), V @ Q[:,:self.rank]
            ...
            if self.N_conserved_basis == 0:
                ...
                # Updating the gloabl rank to coincide with the updated rank
                self.rank = rmax
                ...
                Khat = U @ S
                Khat_ap, Khat_rem = Khat[:,:self.N_conserved_basis], Khat[:,self.N_conserved_basis+1:]; # Splitting Khat into basis required for Ap and remaining vectors
                Vap, Vrem = V[:,:self.N_conserved_basis], V[:,self.N_conserved_basis+1:]; # Splitting Khat into basis required for Ap and remaining vectors
                ...
                S = zeros(rmax+self.N_conserved_basis,rmax+self.N_conserved_basis)
