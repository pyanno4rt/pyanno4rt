...
from numpy import (
    ...
    matmul, maximum, median, ones, sqrt, take, zeros)
...
class LRCMAES:
    ...
    def __init__(
        ...
        self.integrator.initialize_buffers(number_of_variables)
        ...
        self._elite_steps = zeros(
            (self._elite_size, self._number_of_variables),
            order='F', dtype=float64)
        ...
        self._left_basis = eye(
            self._number_of_variables, self.integrator.rank,
            order='F', dtype=float64)
        self._core_matrix = eye(self.integrator.rank, dtype=float64)

        self._core_eig_cache = (
            ones(self.integrator.rank), eye(self.integrator.rank))
        self._root_cov = eye(
            self._number_of_variables, self.integrator.rank,
            order='F', dtype=float64)
    ...
    def S_step(
        ...
        *args : tuple
        ...
        path_cov_projection = left_basis.T @ self._path_cov
        elite_projection = self._elite_steps @ left_basis

        # Add the rank-1 update
        core_matrix += self._lr_rank_1 * (
            path_cov_projection @ path_cov_projection.T)
    ...
    def _update_rank_terms(
        ...
        # Add the rank-1 update
        target_matrix += self._lr_rank_1 * (self._path_cov @ (
            self._path_cov.T @ projection_matrix))

        # Compute the weighted projection of the elite steps
        elite_steps_projection = self._elite_steps @ projection_matrix
        weighted_projection = self._weights_2d * elite_steps_projection

        # Add the rank-mu update
        target_matrix += self._lr_rank_mu * (
            self._elite_steps.T @ weighted_projection)
    ...
    def ask(self):
        ...
        zsamples = self._rng.standard_normal(
            (self._pop_size, self.integrator.rank))

        # Sample steps from the multivariate Gaussian
        matmul(
            zsamples, self._root_cov[:, :self.integrator.rank].T,
            out=self._steps)
    ...
    def tell(
        ...
        self._sigma, elite_indices = _tell(
            ...
            self._core_matrix, self._path_sigma, self._path_cov, self._mean,
            self._sigma, ev, eq, self._lr_sigma, self._lr_cov, self._lr_mean,
            self._mu_eff, self._damp_sigma, self._expected_path_length,
            self._opt_iter, self._elite_size)
        ...
        if self._opt_iter % self._update_interval == 0:
            ...
            self._left_basis, self._core_matrix, _ = self.integrator.update(
                self._left_basis, self._core_matrix, self._left_basis,
                self._lr_cov)
            ...
            matmul(
                self._left_basis, eq * sqrt(ev),
                out=self._root_cov[:, :len(ev)])
...
def _tell(
    ...
    elite_mean_step_tr = left_basis @ (eq @ (inv_root_ev * eq_step))
