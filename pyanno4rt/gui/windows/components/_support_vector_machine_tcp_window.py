...
class SupportVectorMachineTCPWindow(
    ...
    """
    Support vector machine TCP component window for the GUI.

    This class sets up the support vector machine TCP component window for \
    ...
    def save(self):
        ...
        model_parameters = {
            ...
            'tune_space': {
                ...
                'gamma': [
                    2**-15 if self.gamma_lower_bound_ledit.text() == ''
                    else float(self.gamma_lower_bound_ledit.text()),
                    2**3 if self.gamma_upper_bound_ledit.text() == ''
                    else float(self.gamma_upper_bound_ledit.text())],
                'tol': (
        ...
        component = {
            self.segment_cbox.currentText(): {
                ...
                'instance': {
                    'function': 'Support Vector Machine TCP',
                    'parameters': {
        ...
        component_string = ' - '.join((substring for substring in (
            self.segment_cbox.currentText(), 'Support Vector Machine TCP',
    ...
    def close(self):
        """Close the support vector machine TCP window."""
