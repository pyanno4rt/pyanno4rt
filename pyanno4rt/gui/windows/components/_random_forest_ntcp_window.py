"""Random forest NTCP component window."""
...
from pyanno4rt.gui.compilations.components.random_forest_ntcp_window import (
    Ui_random_forest_ntcp_window)
...
class RandomForestNTCPWindow(QMainWindow, Ui_random_forest_ntcp_window):
    """
    Random forest NTCP component window for the GUI.

    This class sets up the random forest NTCP component window for the \
    ...
    def load(
        ...
        for key, value in {
                ...
                'estimators_lower_bound_ledit': (
                    '' if tune_space.n_estimators[0] == 100
                    else str(tune_space.n_estimators[0])),
                'estimators_upper_bound_ledit': (
                    '' if tune_space.n_estimators[-1] == 100
        ...
        self.set_zero_line_cursor((
            'weight_ledit', 'lower_bound_ledit', 'upper_bound_ledit',
            'model_label_ledit', 'model_path_ledit', 'data_path_ledit',
            'prep_steps_ledit', 'estimators_lower_bound_ledit',
            'estimators_upper_bound_ledit', 'max_depth_lower_bound_ledit',
            'max_depth_upper_bound_ledit',
            'min_samples_split_lower_bound_ledit',
            'min_samples_split_upper_bound_ledit',
            'min_samples_leaf_lower_bound_ledit',
    ...
    def save(self):
        ...
        model_parameters = ModelParameters(
            ...
            tune_space=TuneSpaceRF(
                n_estimators=list(range(
                    100 if self.estimators_lower_bound_ledit.text() == ''
                    else int(self.estimators_lower_bound_ledit.text()),
                    101 if self.estimators_upper_bound_ledit.text() == ''
    ...
    def close(self):
        """Close the random forest NTCP window."""
