...
class NeuralNetworkNTCPWindow(
        QMainWindow, Ui_neural_network_ntcp_window):
    """
    Neural network NTCP component window for the application.

    This class creates a neural network NTCP component window for the \
    ...
    def __init__(
        ...
        for box in ('type_cbox', 'embedding_cbox', 'rank_sbox',
                    ...
                    'architecture_cbox', 'max_hidden_layers_sbox',
        ...
        # 
        self.input_activation_lwidget.addItems(['elu', 'gelu', 'leaky_relu',
                                                'linear', 'relu', 'softmax',
                                                'softplus', 'swish'])
        self.hidden_activation_lwidget.addItems(['elu', 'gelu', 'leaky_relu',
                                                 'linear', 'relu', 'softmax',
                                                 'softplus', 'swish'])
        self.optimizer_lwidget.addItems(['Adam', 'Ftrl', 'SGD'])
        self.loss_lwidget.addItems(['BCE', 'FocalBCE', 'KLD'])
        ...
        for index in range(self.input_activation_lwidget.count()):
            self.input_activation_lwidget.item(index).setCheckState(2)
        for index in range(self.hidden_activation_lwidget.count()):
            self.hidden_activation_lwidget.item(index).setCheckState(2)
        for index in range(self.optimizer_lwidget.count()):
            self.optimizer_lwidget.item(index).setCheckState(2)
        for index in range(self.loss_lwidget.count()):
            self.loss_lwidget.item(index).setCheckState(2)
        ...
        self.input_activation_lwidget.setSpacing(4)
        self.hidden_activation_lwidget.setSpacing(4)
        self.optimizer_lwidget.setSpacing(4)
        self.loss_lwidget.setSpacing(4)
    ...
    def load(self, component):
        ...
        self.architecture_cbox.setCurrentText(
            'input-convex' if not model_params.get('architecture')
            else model_params['architecture'])

        self.max_hidden_layers_sbox.setValue(
            2 if not model_params.get('max_hidden_layers')
            else model_params['max_hidden_layers'])
        ...
        self.input_neurons_ledit.setText(
        ...
        for index in range(self.input_activation_lwidget.count()):
            ...
            if (not tune_space.get('input_activation')
                or self.input_activation_lwidget.item(index).text()
                    in tune_space['input_activation']):

                self.input_activation_lwidget.item(index).setCheckState(2)

            else:

                self.input_activation_lwidget.item(index).setCheckState(0)

        self.hidden_neurons_ledit.setText(
        ...
        for index in range(self.hidden_activation_lwidget.count()):
            ...
            if (not tune_space.get('hidden_activation')
                or self.hidden_activation_lwidget.item(index).text()
                    in tune_space['hidden_activation']):

                self.hidden_activation_lwidget.item(index).setCheckState(2)

            else:

                self.hidden_activation_lwidget.item(index).setCheckState(0)

        self.input_dropout_ledit.setText(
            '' if not tune_space.get('input_dropout_rate')
            or tune_space['input_dropout_rate'] == [0.0, 0.1, 0.25, 0.5, 0.75]
            else str(tune_space['input_dropout_rate']))

        self.hidden_dropout_ledit.setText(
            '' if not tune_space.get('hidden_dropout_rate')
            or tune_space['hidden_dropout_rate'] == [0.0, 0.1, 0.25, 0.5, 0.75]
            else str(tune_space['hidden_dropout_rate']))

        self.batch_size_ledit.setText(
            '' if not tune_space.get('batch_size')
            or tune_space['batch_size'] == [4, 8, 16, 32]
            else str(tune_space['batch_size']))

        self.learning_rate_lower_bound_ledit.setText(
            '' if not tune_space.get('learning_rate')
            or tune_space['learning_rate'][0] == 1e-5
            else str(tune_space['learning_rate'][0]))
        self.learning_rate_upper_bound_ledit.setText(
            '' if not tune_space.get('learning_rate')
        ...
        for index in range(self.optimizer_lwidget.count()):
            ...
            if (not tune_space.get('optimizer')
                or self.optimizer_lwidget.item(index).text()
                    in tune_space['optimizer']):

                self.optimizer_lwidget.item(index).setCheckState(2)

            else:

                self.optimizer_lwidget.item(index).setCheckState(0)

        for index in range(self.loss_lwidget.count()):

            if (not tune_space.get('loss')
                or self.loss_lwidget.item(index).text()
                    in tune_space['loss']):
                ...
                self.loss_lwidget.item(index).setCheckState(0)
        ...
        self.set_zero_line_cursor((
            'input_neurons_ledit', 'hidden_neurons_ledit',
            'input_dropout_ledit', 'hidden_dropout_ledit',
        ...
        model_parameters = {
            ...
            'tune_space': {
                ...
                'hidden_activation': [
                    ...
                    for index in range(self.hidden_activation_lwidget.count())
                    if self.hidden_activation_lwidget.item(index).checkState()
                    ],
                'input_dropout_rate': (
                ...
                'learning_rate': (
                    [1e-5, 1e-2] if any(bound == '' for bound in (
                        ...
                        self.learning_rate_upper_bound_ledit.text()))
                          ...
                          float(self.learning_rate_upper_bound_ledit.text())]),
                'optimizer': [
                    self.optimizer_lwidget.item(index).text()
                    for index in range(self.optimizer_lwidget.count())
                    if self.optimizer_lwidget.item(index).checkState()
                    ],
                'loss': [
                    self.loss_lwidget.item(index).text()
                    for index in range(self.loss_lwidget.count())
                    if self.loss_lwidget.item(index).checkState()]},
    ...
    def mousePressEvent(self, event):
        ...
        super(
            QListWidget, self.input_activation_lwidget).mousePressEvent(event)
        super(
            QListWidget, self.hidden_activation_lwidget).mousePressEvent(event)
        super(QListWidget, self.optimizer_lwidget).mousePressEvent(event)
        super(QListWidget, self.loss_lwidget).mousePressEvent(event)
        ...
        if not self.input_activation_lwidget.indexAt(event.pos()).isValid():
            self.input_activation_lwidget.clearSelection()

        if not self.hidden_activation_lwidget.indexAt(event.pos()).isValid():
            self.hidden_activation_lwidget.clearSelection()

        if not self.optimizer_lwidget.indexAt(event.pos()).isValid():
            self.optimizer_lwidget.clearSelection()

        if not self.loss_lwidget.indexAt(event.pos()).isValid():
            self.loss_lwidget.clearSelection()
