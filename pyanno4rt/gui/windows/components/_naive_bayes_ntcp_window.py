"""Naive Bayes NTCP component window."""
...
from pyanno4rt.gui.compilations.components.naive_bayes_ntcp_window import (
    Ui_naive_bayes_ntcp_window)
...
class NaiveBayesNTCPWindow(
        QMainWindow, Ui_naive_bayes_ntcp_window):
    """
    Naive Bayes NTCP component window for the application.

    This class creates a naive Bayes NTCP component window for the \
    ...
    def __init__(
        ...
        # 
        self.priors_none_lwidget.addItem('None')
        ...
        # 
        for index in range(self.priors_none_lwidget.count()):
            self.priors_none_lwidget.item(index).setCheckState(2)
        ...
        self.priors_none_lwidget.setSpacing(4)
    ...
    def load(self, component):
        ...
        self.priors_one_class_ledit.setText(
            '' if not tune_space.get('priors')
            else str(tune_space['priors'][0][1])
            if isinstance(tune_space['priors'][0], list)
            else str(tune_space['priors'][1][1]))

        self.priors_none_lwidget.item(0).setCheckState(
            2 if tune_space.get('priors') and None in tune_space['priors']
            else 0)

        self.var_smoothing_lower_bound_ledit.setText(
            '' if not tune_space.get('var_smoothing')
            or tune_space['var_smoothing'][0] == 1e-12
            else str(tune_space['var_smoothing'][0]))
        self.var_smoothing_upper_bound_ledit.setText(
            '' if not tune_space.get('var_smoothing')
        ...
        self.set_zero_line_cursor((
            'priors_one_class_ledit', 'var_smoothing_lower_bound_ledit',
    ...
    def save(self):
        ...
        model_parameters = {
            ...
            'tune_space': {
                'var_smoothing': (
                    [1e-12, 1] if any(bound == '' for bound in (
                        self.var_smoothing_lower_bound_ledit.text(),
                        self.var_smoothing_upper_bound_ledit.text()))
                    else [
                        float(self.var_smoothing_lower_bound_ledit.text()),
                        float(self.var_smoothing_upper_bound_ledit.text())])},
        ...
        if self.priors_one_class_ledit.text() != '':
            ...
            model_parameters['tune_space']['priors'] = [
                1-float(self.priors_one_class_ledit.text()),
                float(self.priors_one_class_ledit.text())]

        # 
        if self.priors_none_lwidget.item(0).checkState():

            if 'priors' in model_parameters['tune_space']:

                model_parameters['tune_space']['priors'].append(None)

            else:

                model_parameters['tune_space']['priors'] = [None]
    ...
    def mousePressEvent(self, event):
        ...
        super(QListWidget, self.priors_none_lwidget).mousePressEvent(event)
        ...
        if not self.priors_none_lwidget.indexAt(event.pos()).isValid():
            self.priors_none_lwidget.clearSelection()
