...
from pyanno4rt.gui.compilations.components.maximum_dvh_window import (
    Ui_maximum_dvh_window)
from pyanno4rt.gui.styles._custom_styles import ledit, pbutton_composer

# %% Class definition


class MaximumDVHWindow(QMainWindow, Ui_maximum_dvh_window):
    """
    Maximum DVH component window for the application.

    This class creates a maximum DVH component window for the graphical \
    ...
    def __init__(
        ...
        self.target_dose_ledit.textChanged.connect(self.update_save_button)
        self.volume_ledit.textChanged.connect(self.update_save_button)
    ...
    def load(
        ...
        target_dose = component[key]['instance']['parameters']['target_dose']
        volume = component[key]['instance']['parameters']['quantile_volume']
        ...
        # 
        self.target_dose_ledit.setText(str(float(target_dose)))

        # 
        self.volume_ledit.setText(str(float(volume)))
    ...
    def update_save_button(self):
        ...
        if any(text == '' for text in (
                self.segment_cbox.currentText(), self.target_dose_ledit.text(),
