...
class LKBNTCPWindow(QMainWindow, Ui_lkb_ntcp_window):
    ...
    This class creates a LKB NTCP component window for the graphical user \
    interface, including input fields to parametrize the component.
    """
    ...
    def load(self, component):
        ...
        td50 = component[key]['instance']['parameters']['tolerance_dose_50']
        slope = component[key]['instance']['parameters']['slope_parameter']
        ...
        # 
        self.td50_ledit.setText(str(float(td50)))

        # 
        self.slope_ledit.setText(str(float(slope)))
        ...
        self.lower_bound_ledit.setText(
            '' if not lower or float(lower) == 0.0 else str(float(lower)))

        # 
        self.upper_bound_ledit.setText(
            '' if not upper or float(upper) == 1.0 else str(float(upper)))
