"""K-nearest neighbors TCP component window."""
...
from pyanno4rt.gui.compilations.components.k_nearest_neighbors_tcp_window import (
    Ui_k_nearest_neighbors_tcp_window)
...
from pyanno4rt.optimization.components import KNeighborsTCP
from pyanno4rt.tools import string_to_numeric

# %% Class definition


class KNeighborsTCPWindow(QMainWindow, Ui_k_nearest_neighbors_tcp_window):
    """
    K-nearest neighbors TCP component window for the GUI.

    This class sets up the k-nearest neighbors TCP component window for the \
    ...
    def __init__(
        ...
        self.weights_cbox = CheckableComboBox()
        ...
        for box, parameters in {
                'segment_link_cbox': (
                    ...
                    'segment_link_layout'),
                'weights_cbox': (
                    221, ['uniform', 'distance'], True, 'weights_layout'),
                'graphs_cbox': (
        ...
        self.set_styles({
            ...
            'neighbors_lower_bound_ledit': ledit,
            'neighbors_upper_bound_ledit': ledit,
            'weights_cbox': cbox,
            'leaf_size_lower_bound_ledit': ledit,
            'leaf_size_upper_bound_ledit': ledit,
            'p_ledit': ledit,
            'tune_eval_sbox': sbox,
        ...
        for box in (
                ...
                'segment_link_cbox', 'rank_sbox', 'weights_cbox',
        ...
        self.set_zero_line_cursor((
            ...
            'prep_steps_ledit', 'neighbors_lower_bound_ledit',
            'neighbors_upper_bound_ledit', 'leaf_size_lower_bound_ledit',
            'leaf_size_upper_bound_ledit', 'p_ledit', 'identifier_ledit'))
    ...
    def load(
        ...
        for key, value in {
                ...
                'prep_steps_ledit': (
                    ...
                    or model_parameters.preprocessing == ['Identity']
                        ...
                        "\'", '')),
                'neighbors_lower_bound_ledit': (
                    '' if tune_space.n_neighbors[0] == 1
                    else str(tune_space.n_neighbors[0])),
                'neighbors_upper_bound_ledit': (
                    '' if tune_space.n_neighbors[-1] == 10
                    else str(tune_space.n_neighbors[1])),
                'leaf_size_lower_bound_ledit': (
                    '' if tune_space.leaf_size[0] == 1
                    else str(tune_space.leaf_size[0])),
                'leaf_size_upper_bound_ledit': (
                    '' if tune_space.leaf_size[-1] == 500
                    else str(tune_space.leaf_size[-1])),
                'p_ledit': (
                    '' if tune_space.p == [1, 2, 3]
        ...
        for box, selection in {
                ...
                'weights_cbox': tune_space.weights,
    ...
    def save(self):
        ...
        model_parameters = ModelParameters(
            ...
            tune_space=TuneSpaceKNN(
                n_neighbors=list(range(
                    1 if self.neighbors_lower_bound_ledit.text() == ''
                    else int(self.neighbors_lower_bound_ledit.text()),
                    11 if self.neighbors_upper_bound_ledit.text() == ''
                    else int(self.neighbors_upper_bound_ledit.text()))),
                weights=self.weights_cbox.currentData(),
                leaf_size=list(range(
                    1 if self.leaf_size_lower_bound_ledit.text() == ''
                    else int(self.leaf_size_lower_bound_ledit.text()),
                    501 if self.leaf_size_upper_bound_ledit.text() == ''
                    else int(self.leaf_size_upper_bound_ledit.text())+1)),
                p=(
    ...
    def close(self):
        """Close the k-nearest neighbors TCP window."""
