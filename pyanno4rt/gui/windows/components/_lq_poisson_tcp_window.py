...
from pyanno4rt.optimization.components import LQPoissonTCP
...
class LQPoissonTCPWindow(QMainWindow, Ui_lq_poisson_tcp_window):
    """
    LQ Poisson TCP component window for the GUI.

    This class sets up the LQ Poisson TCP component window for the graphical \
    ...
    def __init__(
        ...
        # Set a fixed size
        self.segment_link_cbox.setFixedSize(241, 31)
        ...
        for box in (
                'segment_cbox', 'type_cbox', 'nfx_sbox', 'embedding_cbox',
        ...
        self.set_zero_line_cursor((
            'alpha_ledit', 'beta_ledit', 'vol_eff_ledit', 'weight_ledit',
    ...
    def connect_signals(self):
        ...
        for key, value in {
                'alpha_ledit': self.update_save_button,
                'beta_ledit': self.update_save_button,
    ...
    def load(
        ...
        (segment, alpha, beta, volume_parameter, number_of_fractions,
        component_type, embedding, weight, rank, bounds, link, identifier,
        display) = component.arguments.values()
        ...
        for key, value in {
                'nfx_sbox': int(number_of_fractions),
                'rank_sbox': rank,
                }.items():
    ...
    def save(self):
        ...
        # Get the component
        component = LQPoissonTCP(
            segment=self.segment_cbox.currentText(),
            alpha=string_to_numeric(self.alpha_ledit.text()),
            beta=string_to_numeric(self.beta_ledit.text()),
    ...
    def update_save_button(self):
        ...
        self.save_pbutton.setEnabled(
            all(text != '' for text in (
                    ...
                    self.alpha_ledit.text(),
                    self.beta_ledit.text(),
    ...
    def close(self):
        """Close the LQ Poisson TCP window."""
