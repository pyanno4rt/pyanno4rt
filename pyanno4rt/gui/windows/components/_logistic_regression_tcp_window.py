...
class LogisticRegressionTCPWindow(
    ...
    This class creates a logistic regression TCP component window for the \
    ...
    def __init__(
        ...
        for index in range(self.penalty_lwidget.count()):
            self.penalty_lwidget.item(index).setCheckState(2)
    ...
    def load(self, component):
        ...
        for index in range(self.penalty_lwidget.count()):
            ...
            if (not tune_space.get('penalty')
                or self.penalty_lwidget.item(index).text()
                    in tune_space['penalty']):

                self.penalty_lwidget.item(index).setCheckState(2)

            else:

                self.penalty_lwidget.item(index).setCheckState(0)
